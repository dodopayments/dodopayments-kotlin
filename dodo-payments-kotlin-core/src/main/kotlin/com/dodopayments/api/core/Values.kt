...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString() ?: throw DodoPaymentsInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw DodoPaymentsInvalidDataException("`$name` is not set")
            is JsonNull -> throw DodoPaymentsInvalidDataException("`$name` is null")
            else -> throw DodoPaymentsInvalidDataException("`$name` is invalid, received $this")
        }
