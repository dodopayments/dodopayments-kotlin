...
private constructor(
     ...
     * ```kotlin
     * JsonValue.from("subscription.plan_changed")
     * ```
     *
    ...
    class Builder internal constructor() {
        ...
        private var type: JsonValue = JsonValue.from("subscription.plan_changed")
        private var additionalProperties: MutableMap<String, JsonValue> = mutableMapOf()

        internal fun from(
            subscriptionPlanChangedWebhookEvent: SubscriptionPlanChangedWebhookEvent
        ) = apply {
            businessId = subscriptionPlanChangedWebhookEvent.businessId
            data = subscriptionPlanChangedWebhookEvent.data
            timestamp = subscriptionPlanChangedWebhookEvent.timestamp
    ...
    fun validate(): SubscriptionPlanChangedWebhookEvent = apply {
        ...
        timestamp()
        _type().let {
            if (it != JsonValue.from("subscription.plan_changed")) {
    ...
    internal fun validity(): Int =
        (if (businessId.asKnown() == null) 0 else 1) +
            ...
            type.let { if (it == JsonValue.from("subscription.plan_changed")) 1 else 0 }
    ...
    override fun toString() =
        "SubscriptionPlanChangedWebhookEvent{businessId=$businessId, data=$data, timestamp=$timestamp, type=$type, additionalProperties=$additionalProperties}"
}
